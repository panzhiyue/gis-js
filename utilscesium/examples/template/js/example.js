...
function createGalleryChart(example) {
    var target = "editor.html",
        defaultThumb = "../img/custom/thumb.png",
    ...
    var thumb = $("<img class='chart-thumb' src='" + defaultThumb + "' data-original='" + thumbnail + "' style='display: inline;width:100%;' onerror='this.src=\"img/default.jpg\"'>");
...
function getThumbLocation() {
    ...
    return param.substr(0, decodeURIComponent(param.lastIndexOf('/')));
}

//chart宽高自适应
function resizeCharts() {
    var charts = $("#charts-list .chart");
    var chartsImg = $("#charts-list .chart .chart-thumb");
    if (charts[0] && charts[0].offsetWidth) {
        chartsImg.height(charts[0].offsetWidth * 0.8);
    } else {
        chartsImg.height(260 * 0.8);
    }
...
function scroll() {
    ...
    if (hash && hash.indexOf("#") !== -1) {
        var param = decodeURIComponent(hash.split("#")[1]).split("-");
        if (param.length === 1) {
            ele = $(".category-title#title_" + decodeURIComponent(param[0]));
            selectMenu(param[0], param.length);
        }

        if (param.length == 2) {
            //二级菜单里面的li
            ele = $("#category-type-" + decodeURIComponent(param[1]));
