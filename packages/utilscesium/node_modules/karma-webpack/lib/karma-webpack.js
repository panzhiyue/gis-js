...
/* config.singleRun */singleRun, customFileHandlers, emitter) {
  ...
  applyPlugins.forEach(function (compiler) {
    if (compiler.hooks) {
      compiler.hooks.thisCompilation.tap(this.plugin, function (compilation, params) {
  ...
  if (compiler.hooks) {
    hooks = [{ method: 'sync', name: 'invalid' }, { method: 'async', name: 'watchRun' }, { method: 'async', name: 'run' }];
  }

  hooks.forEach(function (hook) {
    if (compiler.hooks) {
      if (hook.method === 'sync') {
        compiler.hooks[hook.name].tap(this.plugin, function () {
          return handler();
        });
      } else {
        compiler.hooks[hook.name].tapAsync(this.plugin, function (_, callback) {
